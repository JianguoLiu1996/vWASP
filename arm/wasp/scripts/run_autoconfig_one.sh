...
validate_benchmark_config()
{
	...
	if [ $CURR_CONFIG == "F" ] || [ $CURR_CONFIG == "FM" ] || [ $CURR_CONFIG == "FA" ] ||
		[ $CURR_CONFIG == "FAM" ] || [ $CURR_CONFIG == "I" ] || [ $CURR_CONFIG == "IM" ]|| [ $CURR_CONFIG == "IA" ]|| [ $CURR_CONFIG == "IAM" ]; then
...
test_and_set_configs()
{
        ...
        AUTONUMA="0"
        if [ $CURR_CONFIG == "FA" ] || [ $CURR_CONFIG == "FAM" ] || [ $CURR_CONFIG == "TFA" ] || [ $CURR_CONFIG == "TFAM" ]|| [ $CURR_CONFIG == "IA" ] || [ $CURR_CONFIG == "IAM" ]; then
                AUTONUMA="1"
        fi
        ...
        # --- check interleaving
        if [ $CURR_CONFIG == "I" ] || [ $CURR_CONFIG == "IM" ] || [ $CURR_CONFIG == "TI" ] || [ $CURR_CONFIG == "TIM" ]|| [ $CURR_CONFIG == "IA" ] || [ $CURR_CONFIG == "IAM" ]; then
...
launch_benchmark_config()
{
	...
	BENCHMARK_PID=$!
        LAST_CHAR=${CONFIG: -1}
        if [ $LAST_CHAR == "M" ]; then
                AUTOCONFIG_PREFIX+=" -1 0 1 1"
                # $AUTOCONFIG $BENCHMARK_PID > /home/huawei/result/autoconfig.log 2>&1 &
                $AUTOCONFIG $BENCHMARK_PID $AUTOCONFIG_PREFIX > $WASP_OUTFILE 2>&1 &
                AUTOCONFIG_PID=$!
        fi
	...
	SECONDS=0
	$PERF stat -I 10000 -x, -o $OUTFILE --append -e $PERF_EVENTS -p $BENCHMARK_PID &
	PERF_PID=$!
