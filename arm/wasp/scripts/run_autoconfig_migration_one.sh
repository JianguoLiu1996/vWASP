...
# echo "Run one as: $BENCHMARK $CONFIG"
...
prepare_basic_config_params()
{
	CURR_CONFIG=$1
        ...
        LAST_CHAR2=${CURR_CONFIG: -1}
        if [ $LAST_CHAR2 == "A" ]; then
                CURR_CONFIG=${CURR_CONFIG::-1}
        fi
...
prepare_all_pathnames()
{
	...
	NUMACTL=$ROOT"/bin/numactl"
        ...
        if [ $CONFIG == "RPILDM" ] || [ $FIRST_CHAR == "T" ]|| [ $FIRST_CHAR == "RPI-LD+A" ]|| [ $FIRST_CHAR == "RPI-LD+AM" ]; then
                DIR_SUFFIX=10
...
set_system_configs()
{
        ...
        AUTONUMA="0"
        if [ $CURR_CONFIG == "RPILDA" ] || [ $CURR_CONFIG == "RPILDAM" ]; then
                AUTONUMA="1"
        fi
...
launch_benchmark_config()
{
	...
	BENCHMARK_PID=$!
        if [ $LAST_CHAR == "M" ]; then
                ...
                # $AUTOCONFIG $BENCHMARK_PID $AUTOCONFIG_PREFIX> /home/huawei/result/autoconfig.log 2>&1 &
