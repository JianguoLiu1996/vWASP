...
validate_benchmark_config()
{
	...
	if [ $CURR_CONFIG == "LPLD" ] || [ $CURR_CONFIG == "LPRD" ] || [ $CURR_CONFIG == "LPRDI" ] || [ $CURR_CONFIG == "LPLDI" ]||
...
prepare_basic_config_params()
{
	...
	# --- setup interference node
	INT_NODE=0
        if [ $CURR_CONFIG == "LPRDI" ] || [ $CURR_CONFIG == "RPLDI" ] || [ $CURR_CONFIG == "LPLDI" ]; then
                echo "qu : INT_NODE is 0"
                INT_NODE=0
        fi
...
set_system_configs()
{
        ...
        #                 echo $RUNTIMES 
        #                 INT_NODEStr+="-"
        #                 INT_NODEStr+=$RUNTIMES
        #         fi
        #         echo "INT_NODEStr is $INT_NODEStr"
        #         CMD_INI="$NUMACTL -C $INT_NODEStr -m $INT_NODE $INT_BIN"
        #         echo $CMD_INI
...
launch_interference()
{
	...
	if [ $CURR_CONFIG == "LPRDI" ] || [ $CURR_CONFIG == "LPLDI" ] || [ $CURR_CONFIG == "RPILD" ] || [ $CURR_CONFIG == "RPIRDI" ] || [ $CURR_CONFIG == "RPLDI" ]; then
		INT_NODEStr=0
                if [ $RUNTIMES -eq 1 ]; 
                then
                        echo "RUNTIMES = 1"
                ...
                CMD_INI="$NUMACTL -C $INT_NODEStr -m $INT_NODE $INT_BIN"
                echo $CMD_INI
                $NUMACTL -C $INT_NODEStr -m $INT_NODE $INT_BIN > /dev/null 2>&1 &
		if [ $? -ne 0 ]; then
...
launch_benchmark_config()
{
	...
	wait $PERF_PID
        ...
        echo -e "\e[kill icollector"
        wait $icollector_pid
        kill $BENCHMARK_PID &> /dev/null
        echo -e "\e[kill BENCHMARK_PID"
