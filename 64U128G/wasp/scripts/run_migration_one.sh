...
XSBENCH_ARGS=" -- -t 1 -g 40000 -p 1500000" #师兄的配置
#XSBENCH_ARGS=" -- -t 16 -g 50000 -p 4000000 " # my config
...
launch_interference()
{
	...
	if [ $CURR_CONFIG == "LPLDI" ] || [ $CURR_CONFIG == "LPRDI" ] || [ $CURR_CONFIG == "RPILD" ] || [ $CURR_CONFIG == "RPIRDI" ] || [ $CURR_CONFIG == "RPLDI" ]; then
		echo "set interferance!"
                ...
                $NUMACTL -N 3 -m 3 $INT_BIN > /dev/null 2>&1 &
		if [ $? -ne 0 ]; then
